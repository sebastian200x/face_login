...
def login():
    ...
    if request.method == "POST":
        ...
        if user[4] == "yes":
            ...
            session["fullname"] = str(user[10]) + " " + str(user[12])
            ...
            return redirect(url_for("members_"))
...
def register():
    if request.method == "POST":
        ...
        cursor.execute(
            "SELECT username FROM tbl_useracc WHERE username = %s", (input_username,)
        )
        ...
        if username is not None:
            ...
            cursor.execute(
                "INSERT INTO tbl_useracc (username, password) VALUES (%s, %s)",
    ...
    if request.method == "POST":
        ...
        if results[0]:
            ...
            if username_admin is not None:
                ...
                return render_template("camera.html", message=4)  # User 'admin' not found
        else:
